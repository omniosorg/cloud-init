...
def zeropad_mac(mac):
    return ":".join(octet.zfill(2) for octet in mac.split(":"))
...
def illumos_linkprop(devname, prop):
    (out, _err) = subp.subp(
        ["/usr/sbin/dladm", "show-link", "-p", "-o", prop, devname]
    )
    return out.strip()
...
def illumos_intf_in_use(devname):
    (out, _err) = subp.subp(
        ["/usr/sbin/ipadm", "show-addr", "-p", "-o", "addrobj"]
    )
...
def get_default_gateway_on_illumos():
    try:
        ...
        m = re.search(r"^\s+gateway:\s+([0-9.]+)", out, re.MULTILINE)
        if m:
            return m.group(1)
    except (subp.ProcessExecutionError, OSError):
        pass
    return None
...
def _rename_interfaces(
    ...
    for mac, new_name, driver, device_id in renames:
        ...
        if util.is_illumos():
            if not illumos_intf_in_use(cur_name):
                subp.subp(
                    ["/usr/sbin/dladm", "rename-link", cur_name, new_name],
                    capture=True,
                )
            continue
...
def get_interface_mac(ifname):
    ...
    if util.is_illumos():
        (out, _) = subp.subp(
            ["/usr/sbin/dladm", "show-phys", "-m", "-o", "ADDRESS", ifname]
        )
...
def get_interfaces_by_mac_on_illumos() -> dict:
    ret = {}
    (out, _) = subp.subp(
        ["/usr/sbin/dladm", "show-phys", "-m", "-o", "LINK,ADDRESS"]
    )
    for line in out.splitlines():
        (link, mac) = line.split()
        if ":" not in mac:
