...
class Distro(distros.Distro):
    ...
    def __init__(self, name, cfg, paths):
        ...
        self.networking_cls = illumosNetworking
    ...
    def _dhcpattr(attr):
        try:
            (out, _err) = subp.subp(["/sbin/dhcpinfo", attr])
            return out.strip()
        except:
            return None

    @staticmethod
    def obtain_dhcp_lease(
        nic: str,
    ) -> list:
        instance = 'ephdhcp'
        illumosNetOps._ipadm(nic, "create-if", rcs=[0,1])
        illumosNetOps._ipadm(nic, instance=instance,
            cmd=["create-addr", "-T", "dhcp", "-w", "15"])

        lease = [{
            'interface': nic,
            'fixed-address': Distro._dhcpattr('Yiaddr'),
            'subnet-mask': Distro._dhcpattr('Subnet'),
            'router': Distro._dhcpattr('Router'),
        }]
        illumosNetOps._ipadm(nic, instance=instance, cmd="delete-addr")
        return lease if lease[0]['fixed-address'] else None
