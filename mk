...
[ "$1" = clean ] && rm -rf $PWD/root $PWD/build
# Upstream switched from setup.py to meson (PEP 632)
[ -d $PWD/build ] || meson setup build --prefix=/usr -Dinit_system=smf
meson install -C build --destdir=$PWD/root
